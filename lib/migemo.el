...
;; $Id: migemo.el.in,v 1.30 2003/09/26 12:37:04 shirai Exp $
...
(defcustom migemo-directory "/usr/local/share/migemo"
...
(defmacro migemo-if (cond then &rest else)
  "Like `if', except that it evaluates COND at compile-time."
  (if (eval cond) then (` (progn  (,@ else)))))
(put 'migemo-if 'lisp-indent-function 2)

(defvar migemo-poor-emacs nil)

(migemo-if (fboundp 'replace-in-string)
    (defalias 'migemo-replace-in-string 'replace-in-string)
  (defun migemo-replace-in-string (string regexp rep &optional literal)
    "Replace all matches for REGEXP with REP in STRING."
    (if migemo-poor-emacs
...
(defun migemo-backward (word &optional bound noerror count)
  ...
  (if (null migemo-do-isearch)
    ...
    (or (and (not (eq this-command 'isearch-repeat-backward))
	     ...
	     (or (looking-at migemo-search-pattern)
