...
;; Version: $Id: lookup-types.el,v 1.9 2004/01/28 00:09:34 kazuhiko Exp $
...
(cond
 ((featurep 'xemacs)
  (defun lookup-inline-image-p (type)
    "Returns non-nil when images will be displayed under the present
circumstances."
  ...
  (defun lookup-glyph-insert (glyph)
    (put-text-property (1- (point)) (point) 'end-glyph glyph))

  (defun lookup-img-file-insert (file type)
    (lookup-glyph-insert (make-glyph (vector type ':file file))))
  )
...
the present circumstances. TYPE is a symbol like `xbm' or `jpeg'."
 ...
 (t
  (defun lookup-inline-image-p (type)
    "Returns non-nil if the images of type TYPE will be displayed under
