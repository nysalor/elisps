...
;; Version: $Id: lookup-content.el,v 1.2 2004/01/27 09:09:49 kazuhiko Exp $
...
(unless lookup-content-mode-map
  ...
  (define-key lookup-content-mode-map "\C-i" 'lookup-content-next-tab-stop)
  (define-key lookup-content-mode-map [(shift tab)] 'lookup-content-previous-tab-stop)
...
(defun lookup-content-help-echo (&optional pos)
  "Display the help echo string at point."
  (interactive "d")
  (let ((str (get-text-property pos 'help-echo)))
    (unless str
      (let ((link (lookup-get-link pos)))
	(if link (setq str (lookup-entry-id link)))))
    (if str (message str))))

(defun lookup-content-next-tab-stop (&optional nowrap)
  "Jump to the next tab stop point, usually a link."
  (interactive "P")
  (let ((pos (lookup-next-tab-stop)))
    (unless pos
      (if nowrap
	  (error "No more link in this buffer"))
      (save-excursion
	(goto-char (point-min))
	(setq pos (lookup-next-tab-stop)))
      (if (null pos)
	  (error "No link in this buffer"))
      (if (= pos (point))
	  (error "No more link in this buffer")))
    (goto-char pos)
    (lookup-content-help-echo pos)))

(defun lookup-content-previous-tab-stop (&optional nowrap)
  "Jump to the previous tab stop point, usually a link."
  (interactive "P")
  (let ((pos (lookup-previous-tab-stop)))
    (unless pos
      (if nowrap
	  (error "No more link in this buffer")
	(save-excursion
	  (goto-char (point-max))
	  (setq pos (lookup-previous-tab-stop)))
	(if (null pos)
	    (error "No link in this buffer"))
	(if (= pos (point))
	    (error "No more link in this buffer"))))
