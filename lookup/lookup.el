...
;; Version: $Id: lookup.el,v 1.6 2004/01/27 09:09:49 kazuhiko Exp $
...
(eval-when-compile
  (defvar lookup-logo-file (if (or (featurep 'xemacs)
...
(defun lookup-splash ()
  ...
  (when (and lookup-enable-splash window-system
 	     (or lookup-use-bitmap (featurep 'image) (featurep 'xemacs)))
    (let ((buffer (get-buffer-create "*Lookup Logo*")))
      ...
      (erase-buffer)
       (cond ((featurep 'xemacs)
	      (let ((logo-glyph
		     (make-glyph (vector 'xpm :data lookup-logo-image)))
		    logo-ext)
	 ...
	 (insert (format "\nLookup %s\n\n" lookup-version))
	 (insert "Copyright (C) 2000, 2001 Lookup Development Team")
	 (insert "\n<lookup@ring.gr.jp>\n")
